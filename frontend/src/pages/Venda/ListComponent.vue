...
export default {
  ...
  methods: {
    ...
    editar(id) {
      this.$router.push(`/venda/detalhes/${id}`);
    },
    cadastrar() {
      this.$router.push(`/venda/detalhes/nova`);
    },
    async remover(id) {
      try {
    ...
    async getAll() {
      try {
        ...
        this.$router.push({ path: `/venda`, query: params });
    ...
    buildParams() {
      ...
      if (this.selectedAgrupar) {
        params.agrupar_cliente = this.selectedAgrupar;
      }
      if (this.handlePages) {
...
</script>../../../shared/api/vendas/VendasService../../../shared/services/api/vendas/VendasService
